#include <util/uacpi/internal/opregion.h>
#include <util/uacpi/internal/namespace.h>
#include <util/uacpi/internal/utilities.h>
#include <util/uacpi/internal/helpers.h>
#include <util/uacpi/internal/log.h>
#include <util/uacpi/internal/io.h>
#include <util/uacpi/kernel_api.h>
#include <util/uacpi/uacpi.h>
    ...
    uacpi_region_op op, uacpi_region_rw_data *data)
{
    ...
    return op == UACPI_REGION_OP_READ ? uacpi_pci_read(dev, offset, width, &data->value) : uacpi_pci_write(dev, offset, width, data->value);
}
    ...
    uacpi_region_op op, uacpi_region_rw_data *data)
{
    ...
    return op == UACPI_REGION_OP_READ ? uacpi_system_memory_read(ctx->virt, offset, data->byte_width, &data->value) : uacpi_system_memory_write(ctx->virt, offset, data->byte_width, data->value);
}
    ...
    uacpi_region_op op, uacpi_region_rw_data *data)
{
    ...
    return op == UACPI_REGION_OP_READ ? uacpi_system_memory_read(addr, 0, data->byte_width, &data->value) : uacpi_system_memory_write(addr, 0, data->byte_width, data->value);
}
    ...
    uacpi_region_op op, uacpi_region_rw_data *data)
{
    ...
    return op == UACPI_REGION_OP_READ ? uacpi_system_io_read(ctx->handle, offset, width, &data->value) : uacpi_system_io_write(ctx->handle, offset, width, data->value);
}
